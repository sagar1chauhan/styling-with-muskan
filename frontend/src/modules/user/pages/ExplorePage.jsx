    ...
    return (
        <div className="min-h-screen bg-background flex flex-col h-screen overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* Vertical Sidebar */}
                <aside className="w-[80px] bg-accent/30 border-r border-border flex flex-col items-center py-6 gap-6 flex-shrink-0">
                    {SERVICE_TYPES.map((type) => (
                        <button
                            key={type.id}
                            onClick={() => handleTypeChange(type.id)}
                            className={`flex flex-col items-center gap-1.5 transition-all relative ${activeType === type.id ? "opacity-100" : "opacity-40 grayscale"}`}
                        >
                            <div className={`w-14 h-14 rounded-[20px] flex items-center justify-center text-2xl transition-all shadow-sm border-2 ${activeType === type.id ? "bg-white border-primary glow-primary scale-110" : "bg-white/50 border-transparent"}`}>
                                {type.icon}
                            </div>
                            <span className={`text-[10px] font-black uppercase tracking-tight ${activeType === type.id ? "text-primary" : "text-muted-foreground"}`}>
                                {type.label}
                            </span>
                        </button>
                    ))}
                ...
                {/* Main Content Area */}
                <main className="flex-1 flex flex-col overflow-hidden bg-accent/10">
